...
function  stokBt(obj,id,count) {
	
	var addcount=prompt("下架数量","1");
	if(addcount==null)
		return ;
	...
	alert("下架成功");
	obj.href="jspdo/stokDo.jsp?id="+id+"&count="+addcount;
}
...
function delBt(obj , id)
{
	if(confirm("确定移除?"))
	{
		obj.href="jspdo/remShelfDo.jsp?id="+id;	
	}
	return ;
	...
	if (flage != null && flage.equals("find")) {
		...
		switch (typeFlage) {
			...
			case 1 :
				...
				arrayList = goodOfShelfDao.getGoodsByName(name2);
			...
			case 2 :
				...
				Good good2 = goodOfShelfDao.getGoodById(id2);
			...
			case 3 :
				...
				arrayList = goodOfShelfDao.getGoodsByCount(count2);

				break;
			case 4 :
				String pointString = request.getParameter("point");
				int point2 = Integer.parseInt(pointString);
				arrayList = goodOfShelfDao.getGoodsByPoint(point2);
		...
		int begin = --pageInt * 6;
		localPage = begin / 6 + 1;
		arrayList = goodOfShelfDao.getGoods(begin);
		count = goodOfShelfDao.getGoodCount();
		pages = count / 7 + 1;
...
function find() {
	...
	if(goodname.value!="")
	{
		location.href ="showShelf.jsp?flage=find&type=name&name="+goodname.value;
		return ;
	}
	...
	if(goodid.value!="")
	{
		...
		location.href ="showShelf.jsp?flage=find&type=id&id="+goodid.value;
		return ;
	}
	...
	if(goodcount.value!="")
	{
		...
		location.href ="showShelf.jsp?flage=find&type=count&count="+goodcount.value;
		return ;
	}
	...
	if(goodpoint.value!="")
	{
		...
		location.href ="showShelf.jsp?flage=find&type=point&point="+goodpoint.value;
		return ;
	}
}
				...
				<td>
					<%
						out.print(good.count);
							if (good.count <= 10)
								out.print("&nbsp&nbsp<font color='red'>数量少</font>");
					%>
				</td>
				<td><a href="#"
					onclick=" stokBt(this,<%=good.id%>,<%=good.count%>)"
					class="tablelink">下架</a> <a href="#"
	...
	<div class="pagin">
		...
		<ul class="paginList">
			...
			<li class="paginItem current"><a
				href="showShelf.jsp?flage=page&page=<%=i%>"><%=i%></a></li>
			<%
				}
			%>
